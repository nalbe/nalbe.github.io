...
| [aw2033-driver](https://github.com/nalbe/aw2033-driver) | C | Standalone AW2033 RGB LED chip controller: register map + on-chip pattern engine, and the awctl CLI for hand-tuning the LED on-device. Linked by the LED daemon as a static library. |
| [noty-bridge](https://github.com/nalbe/noty-bridge) | Kotlin | Headless NotificationListenerService routing Android notification events to any consumer over abstract Unix sockets - config-driven rules and sinks, decoupled from the LED daemon. |
